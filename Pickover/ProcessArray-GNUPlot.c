...
#define COLORMAP04
  ...
  */
  ...
  OpenPlotter(width, height);
  for (ny=0; ny<height; ny++)
    for (nx=0; nx<width; nx++) {

      /* set color */

      pl_pencolor((int)RealColorRed[Fractal[nx][ny]],
		  (int)RealColorGreen[Fractal[nx][ny]],
		  (int)RealColorBlue[Fractal[nx][ny]]);
      pl_fillcolor((int)RealColorRed[Fractal[nx][ny]],
		   (int)RealColorGreen[Fractal[nx][ny]],
		   (int)RealColorBlue[Fractal[nx][ny]]);
