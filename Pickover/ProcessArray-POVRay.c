...
#define COLORMAP14
  ...
  */
  ...
  /* spit out the POVRay include file */

  strcpy(OutputFileName,FileBase);
  strcat(OutputFileName,".povinc");

  if ( (int)(POVFile=fopen(OutputFileName,"w")) == -1 ) { 
    printf("Could not open POVFile - exiting.\n");
    exit(-1);
  }
...
#define SPHERES
  ...
  for (ny=0; ny<height; ny++)
    for (nx=0; nx<width; nx++) {
      ...
      /*
	for 512x381:
	  for just spheres - .48 radius
	  for height-based spheres:
	    .25 and 5
	    .8 and 100
      */
      ...
      fprintf(POVFile,
	      "sphere { < %d, %f %d>, .6\n",
	      nx, 100.0*(float)Fractal[nx][ny]/(float)MaxUsed, ny);
      /*	      nx, 100.0*(float)Fractal[nx][ny]/(float)MaxUsed, ny);*/
      fprintf(POVFile,"  texture {\n");
      fprintf(POVFile,"    pigment { color red %f green %f blue %f }\n",
	      RealColorRed[Fractal[nx][ny]]/65535.0,
	      RealColorGreen[Fractal[nx][ny]]/65535.0,
	      RealColorBlue[Fractal[nx][ny]]/65535.0);
      fprintf(POVFile,"    finish {\n");

      /* the default, plain, "fast" texture */
      /*
      fprintf(POVFile,"      ambient rgb <%f, %f, %f>\n",
	      RealColorRed[Fractal[nx][ny]]/65535.0,
	      RealColorGreen[Fractal[nx][ny]]/65535.0,
	      RealColorBlue[Fractal[nx][ny]]/65535.0);
      */      
      ...
      fprintf(POVFile,"      diffuse 0.8\n");
      fprintf(POVFile,"      brilliance 5.0\n");


      /*
      fprintf(POVFile,"      specular 0.8\n");
      fprintf(POVFile,"      roughness 0.001\n");
      fprintf(POVFile,"      metallic\n");
      */
	...
	for 512x381:
      */
      ...
      fprintf(POVFile,
	      "box { < %d, 0, %d>, <%d, %f, %d>\n",
	      nx,ny,
	      nx+1, 50.0*(float)Fractal[nx][ny]/(float)MaxUsed, ny+1);
     /*	      nx+1, 100.0*(float)Fractal[nx][ny]/(float)MaxUsed, ny+1);*/
      fprintf(POVFile,"  texture {\n");
      fprintf(POVFile,"    pigment { color red %f green %f blue %f }\n",
	      RealColorRed[Fractal[nx][ny]]/65535.0,
	      RealColorGreen[Fractal[nx][ny]]/65535.0,
  ...
  for (ny=0; ny<height; ny++)
	...
	for 512x381:
      */
      ...
      fprintf(POVFile,
	      "cone { < %d, -0.05, %d>, 0.6, <%d, %f, %d>, 0.25\n",
	      nx,ny,
	      nx, 60.0*(float)Fractal[nx][ny]/(float)MaxUsed, ny);
     /*	      nx, 100.0*(float)Fractal[nx][ny]/(float)MaxUsed, ny);*/
      fprintf(POVFile,"  texture {\n");
      fprintf(POVFile,"    pigment { color red %f green %f blue %f }\n",
	      RealColorRed[Fractal[nx][ny]]/65535.0,
...
#ifdef TORII
  double xrotateinc, zrotateinc;

  xrotateinc=360.0/(double)width;
  zrotateinc=360.0/(double)height;
  ...
  for (ny=0; ny<height; ny++)
	...
	for 512x381:
      */
      ...
      fprintf(POVFile,
	      "torus { 0.8, 0.2 \n");
      fprintf(POVFile,"  rotate <%f, 0.0, %f> \n",
	      (float)((double)nx*xrotateinc),
	      (float)((double)ny*zrotateinc)  );
      fprintf(POVFile,"  translate  <%d, %f, %d> \n",
     	      nx, 100.0*(float)Fractal[nx][ny]/(float)MaxUsed, ny);
      fprintf(POVFile,"  texture {\n");
      fprintf(POVFile,"    pigment { color red %f green %f blue %f }\n",
	      RealColorRed[Fractal[nx][ny]]/65535.0,
  ...
  for (ny=0; ny<height; ny++)
	...
	for 512x381:
      */
      ...
      fprintf(POVFile,"triangle { <%d, %f, %d>, <%d, %f, %d>, <%d, %f, %d> \n",
     	      nx, HeightScale*(float)Fractal[nx][ny]/(float)MaxUsed, ny,
     	      nx+1, HeightScale*(float)Fractal[nx+1][ny]/(float)MaxUsed, ny,
     	    nx+1, HeightScale*(float)Fractal[nx+1][ny+1]/(float)MaxUsed, ny+1);
      fprintf(POVFile,"  texture {\n");
      fprintf(POVFile,"    pigment { color red %f green %f blue %f }\n",
	      RealColorRed[Fractal[nx][ny]]/65535.0,
	      RealColorGreen[Fractal[nx][ny]]/65535.0,
      ...
      */
      ...
      fprintf(POVFile,"triangle { <%d, %f, %d>, <%d, %f, %d>, <%d, %f, %d> \n",
     	      nx, HeightScale*(float)Fractal[nx][ny]/(float)MaxUsed, ny,
     	      nx, HeightScale*(float)Fractal[nx][ny+1]/(float)MaxUsed, ny+1,
   	    nx+1, HeightScale*(float)Fractal[nx+1][ny+1]/(float)MaxUsed, ny+1);
      fprintf(POVFile,"  texture {\n");
      fprintf(POVFile,"    pigment { color red %f green %f blue %f }\n",
	      RealColorRed[Fractal[nx][ny]]/65535.0,
...
#ifdef COLORMAP14  /* fire attempt 2 */
#define TRICOLOR

  /* burnt red - 24415, 7967, 1542 */
  /* orange - 64764, 29555, 15677 */
  /* red - 65535, 3084, 7242 */
  /* new orange - 65535, 23040, 0 */

  strcat(FileBase,"-CMap14");

  RedStart1=65535.0; GreenStart1=3084.0; BlueStart1=7242.0;
  /*  RedEnd1=24415.0; GreenEnd1=7967.0; BlueEnd1=1542.0;*/
  /*  RedEnd1=24415.0; GreenEnd1=7967.0; BlueEnd1=1542.0;*/
  RedEnd1=44415.0; GreenEnd1=7967.0; BlueEnd1=1542.0;

  RedStart2=44415.0; GreenStart2=7967.0; BlueStart2=1542.0;
  /*  RedStart2=24415.0; GreenStart2=7967.0; BlueStart2=1542.0;*/
  RedEnd2=65535.0; GreenEnd2=23040.0; BlueEnd2=0.0;

  RedStart3=65535.0; GreenStart3=23040.0; BlueStart3=0.0;
  RedEnd3=65535.0; GreenEnd3=38656.0; BlueEnd3=0.0;
  /*  RedEnd3=65535.0; GreenEnd3=47872.0; BlueEnd3=0.0;*/
...
#ifdef COLORMAP15 /* fire attempt 3 */
#define TRICOLOR
  ...
  strcat(FileBase,"-CMap15");
  RedStart1=65535.0; GreenStart1=23040.0; BlueStart1=0.0;
  RedEnd1=64764.0; GreenEnd1=29555.0; BlueEnd1=15677.0;
  ...
  RedStart3=65535.0; GreenStart3=3084.0; BlueStart3=7242.0;
  RedEnd3=24415.0; GreenEnd3=7967.0; BlueEnd3=1542.0;
