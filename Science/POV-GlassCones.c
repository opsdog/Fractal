...
#define COLORMAP04
  ...
  */
  ...
  fprintf(POVFile,"light_source { <0, 500, 0> White}\n");
  ...
  for (ny=0; ny<height-1; ny++)
	...
	printf("  %d ",Fractal[nx][ny]);
      ...
      fprintf(POVFile,
	      ...
	      "cone { <%f, 0, %f>, .8 <%f, %f, %f>, .2 texture { pigment { color rgb <%f, %f, %f> transmit 0.25 } finish { ambient .8 diffuse .2 } } }\n",
	      ( (float)nx+.5 ),( (float)ny+.5) ,
	      ( (float)nx+.5 ),
