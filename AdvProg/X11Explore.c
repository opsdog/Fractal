...
/*    nx=204; ny=156; maxiter=1500;*/
    nx=512; ny=381; maxiter=2000;
/*    nx=768; ny=576; maxiter=2000;*/
/*    nx=1020; ny=760; maxiter=2000;*/
...
void save_screen(nx,ny)
int nx,ny;
{
  int row, col, color, scale_factor;
  FILE *fsave;

  fsave=fopen("DougeeTest.pc","wb");
/*  fprintf(fsave,"%d ",nx);*/
/*  fprintf(fsave,"%d ",ny);*/
/*  fprintf(fsave,"%d ",num_entries_colormap);*/
  fwrite(&nx,4,1,fsave);
  fwrite(&ny,4,1,fsave);
  fwrite(&num_entries_colormap,1,4,fsave);
  for (col=0;col<=nx;col++) {
    for (row=0;row<=ny;row++) {
      color=255-MSet[row][col];
/*      fprintf(fsave,"%d ",color);*/
      fwrite(&color,4,1,fsave);
    }
  }
  fclose(fsave);
}
